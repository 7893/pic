...
│  Llama 3.2 扩展查询词 + 自动翻译为英文          │
│  "日落" → "sunset, golden hour, warm sky, dusk"  │
└──────────────────────┬──────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────┐
│  Stage 2: Vector Search                         │
│  BGE Large 1024d embedding → Vectorize topK=100 │
│  毫秒级余弦相似度检索                            │
└──────────────────────┬──────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────┐
│  Stage 3: LLM Re-ranking                        │
│  Llama 3.2 对 top 50 候选重新排序               │
│  理解语义细微差别，纠正向量误匹配                │
└──────────────────────┬──────────────────────────┘
                       ▼
                   搜索结果
...
```
Cron (每小时) → lens-processor
  │
  ├─ Unsplash API → 30 张随机图
  ├─ Queue → LensIngestWorkflow (每张图独立执行)
  │    ├─ 下载原图 + 展示图 → R2
  │    ├─ Llama 3.2 Vision → 结构化描述 + 标签
  │    ├─ BGE Large → 1024d 向量 (融合全部元数据)
  │    └─ 写入 D1 (元数据 + 向量)
  │
  └─ 同步 D1 向量 → Vectorize (幂等 upsert)
```
