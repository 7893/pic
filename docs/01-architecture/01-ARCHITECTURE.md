...
- **调度**: 每 10 分钟触发的 Cron 定时任务（配额不足时自动跳过）。
...
| **查询扩展 (LLM)**      | `Llama 4 Scout 17B`         | 多语言支持强，场景描述更具象化。 |
| **结果重排 (Reranker)** | `BGE Reranker Base`         | 专用重排模型，精度高，速度快。       |
| **向量化 (Embedding)**  | `BGE-M3`                    | 1024 维多语言模型，兼顾精度与检索性能。    |
